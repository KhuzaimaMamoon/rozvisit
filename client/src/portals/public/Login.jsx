    ...
    <main className="min-h-screen bg-background px-4 py-8 sm:px-6 sm:py-12">
      <div className="mx-auto grid max-w-4xl items-center gap-10 lg:grid-cols-[minmax(0,480px)_1fr]">
        <section className="rounded-lg border border-border bg-surface p-6 shadow-sm sm:p-8">
          ...
          <h1 className="mt-7 text-3xl font-semibold tracking-tight text-text">Welcome back</h1>
          <p className="mt-2 text-sm leading-6 text-muted">
            Sign in to continue to your care portal.
          </p>
          {error ? (
            <p
              className="mt-6 border-l-[3px] border-emergency bg-emergency-soft px-4 py-3 text-sm text-emergency"
              role="alert"
            >
              {error}
            </p>
        ...
        <aside className="hidden space-y-5 text-sm leading-6 text-muted lg:block">
          ...
          <p>See the information that matters for your role, without unnecessary complexity.</p>
          <p>Caregivers only see assigned visits after their verification is complete.</p>
        </aside>
      </div>
    </main>
  );
