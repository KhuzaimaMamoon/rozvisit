...
| Stored | Portal memory only — a JavaScript variable, never localStorage, never a cookie (Doc 09 §13). Page reload loses it by design; the first-party refresh cookie restores it silently through `https://rozvisit.com/api/v1/auth/refresh`, which Vercel proxies to the Render API |
...
- **Delivery:** Production: `Set-Cookie: refreshToken_<role>=...; Domain=.rozvisit.com; HttpOnly; Secure; SameSite=Lax; Path=/api/v1/auth`, where `<role>` is `client`, `caregiver`, or `admin`. Browser API calls use the first-party `https://rozvisit.com/api/v1/*` path and Vercel forwards them to `https://api.rozvisit.com`; WebKit therefore does not have to accept or return a cross-origin refresh cookie. Local HTTP development omits `Secure` and the shared `Domain`. The `Path` scope is intentionally limited because only refresh and logout consume the cookie; login only sets it, while all protected domain calls use the memory-only Bearer access token. The client sends its current portal role in `X-RozVisit-Portal`, allowing independent role sessions in separate tabs.
