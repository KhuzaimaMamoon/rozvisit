...
- **Decision:** Production browser clients call the first-party `https://rozvisit.com/api/v1/*` path; Vercel rewrites it to `https://api.rozvisit.com/api/v1/*`. Role-scoped refresh cookies use `Domain=.rozvisit.com; HttpOnly; Secure; SameSite=Lax; Path=/api/v1/auth`. Access tokens remain 15-minute, memory-only Bearer tokens. Registration remains sessionless until verification. Render permits only the exact portal origin through credentialed CORS.
- **Alternatives considered:** Call the custom API domain directly — standards-compliant and same-site, but rejected after repeated iOS WebKit session failures because it still leaves a cross-origin cookie path. Store access or refresh tokens in local/session storage — rejected because XSS could read them. Use `SameSite=None` — rejected because it permits broader cross-site cookie behavior and does not solve WebKit's stricter handling reliably.
- **Consequences:** Browser authentication traffic is genuinely same-origin. Vercel must preserve `Set-Cookie` from the upstream response and forward the cookie on refresh/logout; the custom API remains available behind the proxy. `Lax` is sufficient and the cookie remains narrowly scoped to auth endpoints.
...
- **Proxy amendment (23 July 2026):** Live mobile evidence showed the direct same-site subdomain design still failed on WebKit while desktop succeeded. Production browser calls therefore returned to a first-party Vercel rewrite, now targeting the owned `api.rozvisit.com` host. A safe startup event logs the effective cookie attributes so deployment configuration can be verified without exposing token values.
- **WebKit evidence amendment (23 July 2026):** An iPhone-profile WebKit regression run showed a successful login response and refresh-cookie creation followed by failure of the dynamically imported role-portal module. The protected shell then returned to `/login`, matching the reported symptom. Role portals are therefore included in the initial versioned application bundle instead of being fetched as a second post-login module. The service-worker shell cache version is advanced with this change.
- **Navigation ordering amendment (23 July 2026):** The same WebKit trace showed the login response and Bearer-protected follow-up requests begin before the batched authenticated-user state was guaranteed to commit. Login now commits that state synchronously before changing the SPA URL, preventing the protected-route guard from observing a transient logged-out frame.
